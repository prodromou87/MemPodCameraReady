...
\begin{table}[t]
  ...
  \else
  \includegraphics[width=\linewidth]{figures/revised/new/comparison_table.pdf}
  \fi
  \caption{Breakdown of state-of-the-art designs}
  \label{tbl:breakdown}
\end{table}
...
\item [Migration flexibility:] Defines a mapping of slow-memory regions to regions in fast memory that they can migrate to. 
...
The naive remap table design fails simply because pages are allowed to re-migrate -- like page 10 in our previous example -- while the remap table ``assumes'' the content held at a page address matches the page's ID.  There is only one solution to this problem: The migration logic needs to be aware of exactly where each page's contents are located at any given time. Such a requirement can be implemented in various ways:
	...
	\textbf{CAMEO approach:} Migration is restricted in segments called ``Congruence Groups'' but performed at a granularity of a line. In CAMEO's case, lines belonging to the same congruence group do not compete for a fast memory location, instead each access to slow memory triggers migration evicting the currently residing line. Due to the high migration restriction level CAMEO has the same drawbacks as THM.
	...
	\textbf{MemPod approach:} Migration is restricted to take place within a Pod.
...
%\begin{description}
	...
	\textbf{CAMEO approach:} Activity tracking is not required by CAMEO, since migration decisions are reactive to accesses in slow memory (i.e. not based on the hotness of each line).
...
MEA cannot be cached efficiently as it updates \textit{all} counters on an access when its map structure is full. 
...
%\begin{description}
	...
	\textbf{THM and CAMEO} do not introduce a cost for identifying the candidate pages as it follows a deterministic algorithm. The page or line that triggered migration will be the slow memory candidate, while there is exactly one fast page candidate per segment.
