...
We do two comparisons in this Section.  First, we examine the ability of MEA
...
Based on our results, it appears that lbm's phase changes occur periodically with a period slightly larger that our interval. It accesses a small number of pages repeatedly within a given phase and moves on to a different set of pages in the next phase. FC inevitably identifies the small page set as hot and fails in its prediction for the following interval/phase. The few pages that are accessed towards the end of the interval appear to be sufficient for MEA to accurately predict the next phase's hot pages. Based on the official lbm description \cite{SPEC-LBM-DESCRIPTION}, it's reasonable that lbm could have this behavior. To verify our assumptions, we executed the same experiment with double the size of the interval (11000 requests). We observed that the trend remains the same but shifted to lower tiers. In this case both MEA and FC scored zero hits in the top three tiers. However, MEA reports high numbers in tiers 4,5 and 6, while FC still reports zero.
